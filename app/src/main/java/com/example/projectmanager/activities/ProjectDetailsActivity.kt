            ...
            if (projectname != null) {
                ...
                viewModel.getTasks(projectname).observe(this) { tasks ->
                    ...
                    for(i in 0 until tasks.size) {
                        if(tasks[i]["member"]?.contains(username) == true) {
...
//                            outstanding_tasks_list.text = task1
                    ...
                    if(!tasklist.isEmpty()) {
                        val simpleAdapter = SimpleAdapter (
                            ...
                            intArrayOf(R.id.outstanding_tasks_list_name, R.id.task_deadline, R.id.task_status)
                        )
                                    ...
                                    showTwoButtonDialog(
                                        ...
                                        View.OnClickListener {
                                            tasklist[i]["task"]?.let { it1 ->
                                                viewModel.setTaskToComplete(projectname, it1)
                                                val intent = Intent(this@ProjectDetailsActivity, ProjectDetailsActivity::class.java)
                                                var bundle = Bundle()
                                        ...
                                        View.OnClickListener {val intent = Intent(this@ProjectDetailsActivity, ProjectDetailsActivity::class.java)
                                            var bundle = Bundle()
                                            bundle.putString("User", username)
                                            bundle.putString("Project", projectname)
                                            intent.putExtras(bundle)
